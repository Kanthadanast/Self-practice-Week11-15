import {
    ...
    addQuote,
    editQuote,
  } from "./week12/quoteManagement.js"
  ...
  function handleEdit(e) {
    ...
    // ดึง content และ author ของ quote ที่กด edit มาใช้
  ...
  async function handleDelete(e) {
    ...
    if (ans) {
      try {
        ...
        console.log(quoteListEle) //ตรวจสอบว่าได้หา div ของ quote ที่จะลบ มาใช้ใน remove มั้ย
  ...
  async function handleAddEdit(event) {
    ...
    if (quoteId) {
      ...
      try {
        ...
        const updateQuoteDivEle = document.querySelector( //ค้นหา div ของ quote ที่แก้ไขได้มาใช้ใน update เพื่อแสดงผลของ quote ที่แก้ไข
          `div[data-id="${updateQuote.id}"]`
        )
        updateQuoteDivEle.children[0].textContent = updateQuote.content
        updateQuoteDivEle.children[1].textContent = updateQuote.author //แสดงผลของ quote ที่แก้ไข
      } catch (e) {
